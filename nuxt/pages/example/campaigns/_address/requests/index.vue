      ...
      <v-flex xs12>
        ...
        >
          <template slot="items" slot-scope="props">
            <td :class="itemClass('text-xs-left', props.item)">
              {{ props.item.id }}
            </td>
            <td :class="itemClass('text-xs-left', props.item)">
              {{ props.item.description }}
            </td>
            <td :class="itemClass('text-xs-right', props.item)">
              {{ toEther(props.item.value) }}
            </td>
            <td :class="itemClass('text-xs-left', props.item)">
              {{ props.item.recipient }}
            </td>
            <td :class="itemClass('text-xs-center', props.item)">
            ...
            <td class="justify-center">
              <v-btn
                ...
                color="success"
                :disabled="
                  (!(props.item.id === currentId && isOnApprove) && loading) ||
                  props.item.approvalCount >= approversCount
                "
            ...
            <td class="justify-center">
              <v-btn
                ...
                color="info"
                :disabled="
                  (!(props.item.id === currentId && isOnfinalize) && loading) ||
...
export default {
  ...
  computed: {
    items() {
      const currentCampaign = this.$store.getters.loadedCampaign(this.address)
  ...
  async mounted() {
    ...
    if (
      (currentCampaign === undefined ||
        currentCampaign.manager === undefined ||
        currentCampaign.requests === undefined) &&
      this.address !== null
    ) {
      if (
  ...
  methods: {
    ...
    toEther(fromWei) {
      return this.$ethereumService.utils.fromWei(fromWei, 'ether')
    },
    readyToFinalize(approvalCount) {
      return approvalCount > this.approversCount / 2
    },
    itemClass(fixclass, item) {
      const color = item.complete
        ? 'grey--text'
        : this.readyToFinalize(item.approvalCount)
        ? 'green--text'
        : ''
