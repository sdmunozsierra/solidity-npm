<template>
  <div id="app">
    ...
    <h1 v-if="$eth.isConnected">
      Connected to {{ $eth.networkId }} {{ $eth.networkName }}
    ...
    <h1 v-else>You are not connected to an Ethereum network</h1>
    <button v-if="!$eth.isConnected" @click="connect()">
      connect to Ethereum
    </button>
    <div v-if="$eth.isConnected">
      ...
      <h2 v-if="$eth.accounts.length">
        Your account<span v-if="$eth.accounts.length > 1">s</span>
      </h2>
      <ul>
